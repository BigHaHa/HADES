        ...
        for (Int_t i=0; i<NC-4;i++){
            for(Int_t j=0; j<NRAPI; j++){
                for(Int_t k=0; k<NPTRN-5; k++){
                    //-proton------------------------------------------
                    pfy0pt[i][j][k].THDeclare("ACent",i,"Yo",j,"Pt",k);
                    pfR0pt[i][j][k].THDeclare("BCent",i,"Yo",j,"Pt",k);
    ...
    for (Int_t i=1; i<events;i++){
        ...
        for(Int_t j=0; j<(wallCat->getEntries()); j++){
            ...
            if(cellTime>T1_cut && cellTime<T2_cut && cellCharge>dEdxCut){
                ...
                for (Int_t im=0;im<11;im++){
                    //-X-beam-center-scan---
                    if( (Mtof+Mrpc)>=  Mrang[im] && (Mtof+Mrpc)< Mrang[im+1] ){ hFWxyCCsmearXscanM[im]->Fill(XfwSmear,YfwSmear); }
                    //-Y-beam-center-scan---
                    if( (Mtof+Mrpc)>=  Mrang[im] && (Mtof+Mrpc)< Mrang[im+1] ){ hFWxyCCsmearYscanM[im]->Fill(XfwSmear,YfwSmear); }
                ...
                if(  wallXc*wallXc + wallYc*wallYc >= R0_cut*R0_cut  /*50.*50.*/ ){
                    ...
                    for (Int_t im=0;im<11;im++){
                        if( (Mtof+Mrpc)>=Mrang[im] && (Mtof+Mrpc)<Mrang[im+1]){ hFWxyCCsmearM[im]->Fill(XfwSmear+X_shift,YfwSmear+Y_shift); }
                    }
                }//-endif-R0_cut--//
            }//end-if-cells-cut---
        ...
        if (Mfw > 2){
            for (Int_t im=0;im<11;im++){
                if((Mtof+Mrpc)>=Mrang[im] && (Mtof+Mrpc)<Mrang[im+1]){
                    ...
                    hQvRaw[ im]->Fill(vsum.Y(),vsum.X()); hQvRec[ im]->Fill(vsumCorr.Y(),vsumCorr.X());
                }
            }
            for (Int_t n=1;n<=6;n++){
                for (Int_t im=0;im<11;im++){
                    if ((Mtof+Mrpc)>=Mrang[im] && (Mtof+Mrpc)<Mrang[im+1]){
                        hSinPsi[n-1][im]->Fill(DAY_NUM,sin(n*VectphiCorr*rad2deg));
                        hCosPsi[n-1][im]->Fill(DAY_NUM,cos(n*VectphiCorr*rad2deg));
                    }
                }
            }
            for (Int_t im=0;im<11;im++){
                for (Int_t n=0;n<6;n++){
                    ...
                    dPsi += 2*(-avSin*cos((n+1)*VectphiCorr*hpi/90.) + avCos*sin((n+1)*VectphiCorr*hpi/90.))/(n+1);
                }
                PsiCorr = atan2(sin((VectphiCorr+dPsi)*hpi/90.),cos((VectphiCorr+dPsi)*hpi/90.))*rad2deg;
                if( (Mtof+Mrpc)>=Mrang[im]    && (Mtof+Mrpc)<Mrang[im+1]    ){ hPsiEP[im]->Fill(VectphiEP); hPsiRcnt[im]->Fill(VectphiCorr); hPsiCorr[im]->Fill(PsiCorr); }
            }
            for (Int_t im=0;im<11;im++){
                if ((Mtof+Mrpc)>=Mrang[im] && (Mtof+Mrpc)<Mrang[im+1]){ hsumXmean[im]->Fill(DAY_NUM,vsum.X()); hsumYmean[im]->Fill(DAY_NUM,vsum.Y()); }
            }
        ...
        for(Int_t j=0;j<size;j++){
            ...
            hphi    -> Fill(phi);
            dphiA     = phi - phiA;
            if (dphiA> 180.){dphiA = dphiA - 360.; }
            if (dphiA<-180.){dphiA = dphiA + 360.; }
            dphiB     = phi - phiB;
            if (dphiB> 180.){dphiB = dphiB - 360.; }
            if (dphiB<-180.){dphiB = dphiB + 360.; }
            ...
            if ( mass>600 && mass<1250 && NA>0 && NB>0 && charge>0 && metaQa<5 && chi2<200. && chi2In>0.1 && chi2In<12. && chi2Out>0.1 && chi2Out<12. && sec>=0 && sec<=5 && mdcdEdx>fdEdxVsMomLowLimit(mom)){
                ...
                if( CENT>=4 && RAPI>=0 && PTRN>=0 && PTRN<=12 ){
                    pfy0pt[ CENT-4][RAPI][PTRN].Fill(dphiA   , fabs(phiAB), phiA  , 1.*TrackEff);
                    pfR0pt[ CENT-4][RAPI][PTRN].Fill(dphiB, fabs(phiAB), phiB, 1.*TrackEff);
